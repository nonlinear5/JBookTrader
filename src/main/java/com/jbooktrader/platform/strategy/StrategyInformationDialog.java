...
public class StrategyInformationDialog extends JBTDialog {
    ...
    private void init() {
        ...
        NumberFormat nf1 = NumberFormatterFactory.getNumberFormatter(1);
        ...
        add(performancePanel, "OG", nf2.format(pm.getOptimalGrowth()));
        add(performancePanel, "PI", nf2.format(pm.getPI()));
        add(performancePanel, "OL", nf1.format(pm.getOptimalLeverage()));
        ...
        add(securityPanel, "Ticker", strategy.getTicker());
        add(securityPanel, "Security Type", strategy.getContract().secType().getApiString());
